...
$sql = "SELECT events.id, events.status, events.title, events.date_created, events.date_edited, events.price, events.img1, events.brief, events_types.title ititle, locations.title ltitle FROM events INNER JOIN events_types ON events.type_id = events_types.id INNER JOIN locations ON events.geo2_id = locations.id WHERE events.member_id=%d AND (events.title LIKE '%s' OR events.keywords LIKE '%s')";

if($search_data_m_status > -1){$sql .= ' AND events.status = ' . $search_data_m_status;}
...
while($row = mysql_fetch_assoc($sqlquery)){ 
	...
	if(strlen($field_t_img1) == 0){$field_t_img1 = 'none.jpg';}
		...
		$finalString .= '<tr>
                      ...
                      <td class="add-img-td">
                        <a href="">
                          <img alt="' . safe_alt($field_t_title) . '" src="files/events/small/' . $field_t_img1 . '" title="' . safe_alt($field_t_title) . '" class="img-responsive">
                        </a>
                      </td>
                      ...
                      <td class="action-td">
                        <a class="btn btn-primary btn-xs" href="my-events-manage/' . $field_t_id . '" onclick="return hs.htmlExpand(this, { objectType: \'iframe\',  objectWidth: 900, objectHeight: 950 } )"> <i class="fa fa-pencil-square-o"></i> Edit</a>
                        <a class="btn btn-info btn-xs"> <i class="fa fa-share-square-o"></i> Share</a>
                        <a class="btn btn-danger btn-xs" href="my-events-delete/' . $field_t_id . '" onclick="if(confirm(\'Are you sure you want to delete this listing?\')){window.location.href=this.href;}else{return false;}"> <i class=" fa fa-trash"></i> Delete</a>
                      </td>
                    </tr>';
	
	
	/*$finalString .= '<div class="addrows' . ($field_t_status == 0 ? ' addrowspending' : '') . '"><span class="addentries addentry1">' . $num_listings . '.</span><span class="addentries addentry2"><img src="files/events/small/' . $field_t_img1 . '" alt="' . safe_alt($field_t_title) . '" title="' . safe_alt($field_t_title) . '" /></span><span class="addentries addentry3">' . $field_t_title . '<span class="addprice">' . $field_t_price . '</span><span class="addrent">' . $field_t_category . '</span></span><span class="addentries addentry4">' . date('d M \'y', $field_t_dateadded) . '</span><span class="addentries addentry5">' . date('d M \'y', $field_t_dateedited) . '</span><span class="addentries addentry6"><a href="my-events-manage/' . $field_t_id . '" onclick="return hs.htmlExpand(this, { objectType: \'iframe\',  objectWidth: 900, objectHeight: 950 } )"><img src="graphics/icons/memberadd-edit.png" alt="Edit" title="Edit" height="16" width="15" /></a>&nbsp;&nbsp;<a href="my-events-delete/' . $field_t_id . '" onclick="if(confirm(\'Are you sure you want to delete this listing?\')){window.location.href=this.href;}else{return false;}"><img src="graphics/icons/memberadd-delete.png" alt="Delete" title="Delete" height="16" width="14" /></a></span></div>';*/
...
if($numResults > $pp){
	...
	$finalPaging .= '<div class="memberpaging"> <span class="total">Page <strong>' . $page . '</strong> of <strong>' . $numPages . '</strong> (' . $numResults . ' listing' . ($numResults == 1 ? '' : 's') . ')</span> <span class="splitter">|</span> <a class="lastlink" href="my-events/1' . $paging_extra . '">First</a> <span class="splitter">|</span>';
	...
	if($numPages > 5){
		...
		for($i = $int__l_bound; $i<=$int__h_bound; $i++){
			if($i == $page){
				$finalPaging .= '<a href="my-events/' . $i . $paging_extra . '" class="selected">' . $i . '</a> ';
			}else{
				$finalPaging .= '<a href="my-events/' . $i . $paging_extra . '">' . $i . '</a> ';
			}
		}
	...
	$finalPaging .= '<span class="splitter">|</span> <a class="lastlink" href="my-events/' . $numPages . $paging_extra . '">Last</a></div>';
