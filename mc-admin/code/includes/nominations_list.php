...
$sqlquery = mysql_query("SELECT categories.*, (SELECT COUNT(id) FROM nominations WHERE nominations.category_id = categories.id) num_recs FROM categories ORDER BY ordering ASC LIMIT " . $lbound . ", " . $pp);
...
while($row = mysql_fetch_array($sqlquery)){ 
	...
	$form_item_num = filter_out($row['num_recs']);
	
	$form_item_data .= '<tr class="' . ($form_item_count % 2 == 1 ? 'first' : 'second') . '"><td>' . $form_item_title . '</td><td style="width: 85px;text-align:center;">' . $form_item_num . '</td><td style="width: 57px;"><div class="actions"><ul><li><a class="a_save" href="nominations/export-' . $form_item_id . '" title="Export"></a></li></ul></div></td></tr>';
