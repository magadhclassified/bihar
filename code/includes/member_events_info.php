...
if($listing_id > 0){
	$sqlquery = mysql_query(sprintf("SELECT * FROM events WHERE id = %d AND member_id = %d",
	...
	while($row = mysql_fetch_array($sqlquery)){ 
		...
		$item_venue = filter_out($row['venue']);
		$item_times = filter_out($row['times']);
		...
		$item_contact_web = filter_out($row['website']);
...
$sqlquery = mysql_query("SELECT id, title FROM events_types ORDER BY title ASC");
