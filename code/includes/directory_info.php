...
$sqlquery = mysql_query(sprintf('SELECT directory.*, locations.title ltitle FROM directory INNER JOIN locations ON directory.geo2_id = locations.id WHERE directory.id = %d AND directory.status = 1',
...
while($row = mysql_fetch_array($sqlquery)){ 
	...
	$url = COMPANY_URL . 'directory-details/' . $listing_id . '/' . seo($field_title);
	...
	$og_image = COMPANY_URL . 'files/directory/thumb/' . $field_img;
	$og_url = $url;
}
...
while($row = mysql_fetch_array($sqlquery)){ 
	$field_uprofile_company = filer_out_limit($row['company_name']);
	$field_uprofile_contact = filer_out_limit($row['contact_person']);
	$field_uprofile_address = filer_out_limit($row['address']);
	$field_uprofile_tel = filer_out_limit($row['tel']);
	$field_uprofile_fax = filer_out_limit($row['fax']);
