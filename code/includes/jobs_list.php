...
if($search_id > 0){
	...
	$search_data_industry = (int)$search_criteria['industry'];
	$search_data_commitment = (int)$search_criteria['commitment'];
	$search_data_type = (int)$search_criteria['type'];
	$search_data_member = (int)$search_criteria['member'];

	$sr_kw = $search_data_kw;
...
$sql = "SELECT jobs.id, jobs.title, jobs.img1, jobs.brief, jobs_industries.title ititle, locations.title ltitle FROM jobs INNER JOIN jobs_industries ON jobs.industry_id = jobs_industries.id INNER JOIN locations ON jobs.geo2_id = locations.id WHERE jobs.status=1 AND jobs_industries.status=1 AND locations.status=1 AND (jobs.title LIKE '%s' OR jobs.keywords LIKE '%s' OR jobs.ref LIKE '%s')";

if($search_data_industry > 0){$sql .= ' AND jobs.industry_id = ' . $search_data_industry;}
if($search_data_commitment > 0){$sql .= ' AND jobs.commitment_id = ' . $search_data_commitment;}
if($search_data_type > 0){$sql .= ' AND jobs.type_id = ' . $search_data_type;}
if($search_data_member > 0){$sql .= ' AND jobs.member_id = ' . $search_data_member;}
...
while($row = mysql_fetch_assoc($sqlquery)){ 
	...
	$url = 'job-details/' . $field_t_id . '/' . strtolower(seo($field_t_title));
	...
	$finalString .= '<div class="item-list">
                <div class="col-sm-2 no-padding photobox">
                  <div class="add-image">
                    <a href="' . $url . '"><img src="files/jobs/thumb/' . $field_t_img1 . '" alt="' . safe_alt($field_t_title) . '" title="' . safe_alt($field_t_title) . '" /></a>
...
if($numResults > $pp){
	...
	$finalPaging .= '<div class="pagination-bar"> <ul class="pagination"><li><a href="jobs/1' . $paging_extra . '" class="pagination-btn">Previous</a></li>';
	...
	if($numPages > 7){
		...
		for($i = $int__l_bound; $i<=$int__h_bound; $i++){
			if($i == $page){
				$finalPaging .= '<li class="active"><a href="jobs/' . $i . $paging_extra . '">' . $i . '</a></li>';
			}else{
				$finalPaging .= '<li><a href="jobs/' . $i . $paging_extra . '">' . $i . '</a></li> ';
			}
		}
		...
		for($i = 1; $i<=$numPages; $i++){
			if($i == $page){
				$finalPaging .= '<li class="active"><a href="jobs/' . $i . $paging_extra . '" class="selected">' . $i . '</a></li> ';
			}else{
				$finalPaging .= '<li><a href="jobs/' . $i . $paging_extra . '">' . $i . '</a></li> ';
			}
		}
	...
	$finalPaging .= '<li><a href="jobs/'. $numPages . $paging_extra .'" class="pagination-btn">Next »</a></li></ul></div>';
