...
for($x=1; $x<=4; $x++){
	if($_FILES['frm_upload' . $x]['size'] > 0){
		...
		if ($handle->uploaded && $handle->file_is_image) {
			...
			header('Location: ../../my-motors-manage');
			exit();
		}
	}
}
...
if($id > 0){
		...
		mysql_query("INSERT INTO logs_events(ip, date_created, user_id, c_action, c_location, c_table, c_id) VALUES ('" . get_the_ip() . "', " . time() . ", 0, 'MEMBER-EDIT', 'Motors (ID: " . $id . "), Member (ID: " . $_SESSION['Member_ID'] . ")', 'motors', " . $id . ")");
	...
	mysql_query("INSERT INTO logs_events(ip, date_created, user_id, c_action, c_location, c_table, c_id) VALUES ('" . get_the_ip() . "', " . time() . ", 0, 'MEMBER-ADD', 'Motors (ID: " . $document_id . "), Member (ID: " . $_SESSION['Member_ID'] . ")', 'motors', " . $document_id . ")");
