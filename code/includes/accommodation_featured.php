...
$sql = "SELECT accommodation.id, accommodation.title, accommodation.price, accommodation.img1, accommodation.brief, accommodation_types.title acctype, locations.title ltitle FROM accommodation INNER JOIN locations ON accommodation.geo2_id = locations.id INNER JOIN accommodation_types ON accommodation.type_id = accommodation_types.id WHERE accommodation.status=1 AND locations.status=1 AND accommodation.is_featured = 1";
if($global_emirate_id > 0){$sql .= ' AND accommodation.geo1_id = ' . $global_emirate_id;}
$sql .= ' ORDER BY accommodation.date_created DESC LIMIT 2';
