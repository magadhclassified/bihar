...
if($id > 0){
	...
	$sqlquery = mysql_query(sprintf("INSERT INTO accommodation(title, price, type_id, rating_id, from_airport, geo1_id, geo2_id, geo3_id, contact_name, contact_number, contact_email, brief, address, keywords, img1, img2, img3, img4, gps_lat, gps_lon, contents, member_id, date_created, date_edited, is_featured, is_fp, status) VALUES('%s', '%s', %d, %d, '%s', %d, %d, %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d, %d, %d, %d, %d)",
				mysql_real_escape_string($title),
				mysql_real_escape_string($price),
				mysql_real_escape_string($type),
				mysql_real_escape_string($rating),
	...
	mysql_query("INSERT INTO logs_events(ip, date_created, user_id, c_action, c_location, c_table, c_id) VALUES ('" . get_the_ip() . "', " . time() . ", " . $_SESSION['Admin_ID'] . ", 'ADD', 'Listings - Accommodation', 'accommodation', " . $document_id . ")");
