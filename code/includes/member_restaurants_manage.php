...
if($id > 0){
		...
		mysql_query("INSERT INTO logs_events(ip, date_created, user_id, c_action, c_location, c_table, c_id) VALUES ('" . get_the_ip() . "', " . time() . ", 0, 'MEMBER-EDIT', 'Restaurants (ID: " . $id . "), Member (ID: " . $_SESSION['Member_ID'] . ")', 'properties', " . $id . ")");
		
		$sqlquery = mysql_query(sprintf("UPDATE restaurants SET title='%s', price='%s', menu_id=%d, dresscode_id=%d, geo1_id=%d, geo2_id=%d, geo3_id=%d, contact_name='%s', contact_number='%s', contact_email='%s', brief='%s', keywords='%s', img1='%s', img2='%s', img3='%s', img4='%s', gps_lat='%s', gps_lon='%s', contents='%s', date_edited=%d, status=%d WHERE id = %d AND member_id = %d",
	...
	$sqlquery = mysql_query(sprintf("INSERT INTO restaurants(title, price, menu_id, dresscode_id, geo1_id, geo2_id, geo3_id, contact_name, contact_number, contact_email, brief, keywords, img1, img2, img3, img4, gps_lat, gps_lon, contents, member_id, date_created, date_edited, is_featured, is_fp, status) VALUES('%s', '%s', %d, %d, %d, %d, %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d, %d, %d, %d, %d)",
				mysql_real_escape_string($title),
				mysql_real_escape_string($price),
				mysql_real_escape_string($menu),
				mysql_real_escape_string($dresscode),
	...
	mysql_query("INSERT INTO logs_events(ip, date_created, user_id, c_action, c_location, c_table, c_id) VALUES ('" . get_the_ip() . "', " . time() . ", 0, 'MEMBER-ADD', 'Restaurants (ID: " . $document_id . "), Member (ID: " . $_SESSION['Member_ID'] . ")', 'restaurants', " . $document_id . ")");
