...
if($id > 0){
		...
		mysql_query("INSERT INTO logs_events(ip, date_created, user_id, c_action, c_location, c_table, c_id) VALUES ('" . get_the_ip() . "', " . time() . ", " . $_SESSION['Admin_ID'] . ", 'EDIT', 'Listings - Events', 'events', " . $id . ")");
		
		$sqlquery = mysql_query(sprintf("UPDATE events SET title='%s', price='%s', type_id=%d, date_from=%d, date_to=%d, venue='%s', times='%s', website='%s', geo1_id=%d, geo2_id=%d, geo3_id=%d, contact_name='%s', contact_number='%s', contact_email='%s', brief='%s', keywords='%s', img1='%s', gps_lat='%s', gps_lon='%s', contents='%s', member_id=%d, date_edited=%d, is_featured=%d, is_fp=%d, status=%d WHERE id = %d",
					mysql_real_escape_string($title),
					mysql_real_escape_string($entryfee),
	...
	mysql_query("INSERT INTO logs_events(ip, date_created, user_id, c_action, c_location, c_table, c_id) VALUES ('" . get_the_ip() . "', " . time() . ", " . $_SESSION['Admin_ID'] . ", 'ADD', 'Listings - Events', 'events', " . $document_id . ")");
