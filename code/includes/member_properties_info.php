...
if($listing_id > 0){
	$sqlquery = mysql_query(sprintf("SELECT * FROM properties WHERE id = %d AND member_id = %d",
...
while($row = mysql_fetch_array($sqlquery)){ 
	...
	if(in_array($item_ad_id, $views_selected)){
		$views_list .= '<option value="' . $item_ad_id . '" selected="selected">&nbsp;' . $item_ad_title . '</option>';
	}else{
		$views_list .= '<option value="' . $item_ad_id . '">&nbsp;' . $item_ad_title . '</option>';
	}
}
...
while($row = mysql_fetch_array($sqlquery)){ 
	...
	if(in_array($item_ad_id, $amenities_selected)){
		$amenities_list .= '<option value="' . $item_ad_id . '" selected="selected">&nbsp;' . $item_ad_title . '</option>';
	}else{
		$amenities_list .= '<option value="' . $item_ad_id . '">&nbsp;' . $item_ad_title . '</option>';
	}
}
